...
test("English prompt includes current Table 2 and instruction requirements", () => {
  ...
  assert.match(prompt, /impact on portfolio risk contribution/);
  assert.match(prompt, /Internal validation \(MANDATORY before final answer\):/);
  assert.match(prompt, /all tables are internally consistent/);
  assert.match(prompt, /minimum position sizes are respected unless explicitly justified/);
  assert.match(prompt, /cannot be simplified further without materially reducing diversification quality or implementation robustness/);
  assert.match(prompt, /Reasoning discipline \(MANDATORY\):\n- Follow the portfolio construction steps in the defined order\.[\s\S]*implementation efficiency\n- Keep reasoning structured, explicit, and decision-oriented\.[\s\S]*Internal validation \(MANDATORY before final answer\):\n- Verify that:/);
  assert.match(prompt, /mean-variance optimisation logic consistent with the efficient frontier/);
  assert.match(prompt, /balancing risk contributions across asset classes\.\n- Apply the following logic conceptually/);
  assert.doesNotMatch(prompt, /Portfolio construction methodology \(MANDATORY\):[\s\S]*Explain why the resulting allocation is close to an efficient portfolio[\s\S]*Eligible asset classes:/);
...
test("German prompt includes current Table 2 and instruction requirements", () => {
  ...
  assert.match(prompt, /Vermeide generische, repetitive oder rein erzählerische Erklärungen/);
  assert.match(prompt, /Interne Validierung \(VERPFLICHTEND vor der finalen Antwort\):/);
  assert.match(prompt, /alle Tabellen intern konsistent sind/);
  assert.match(prompt, /Mindestpositionsgrössen eingehalten werden, sofern Abweichungen nicht explizit begründet sind/);
  assert.match(prompt, /Umsetzungsrobustheit wesentlich zu reduzieren/);
  assert.match(prompt, /Portfolio-Konstruktionsmethodik \(VERPFLICHTEND\):/);
  assert.match(prompt, /Mean-Variance-Optimierungslogik/);
  assert.match(prompt, /Risikobeiträge über Anlageklassen ausbalancierst\.\n- Wende die Logik konzeptionell an/);
