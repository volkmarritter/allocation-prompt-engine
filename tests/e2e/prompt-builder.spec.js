...
test.describe("Portfolio Prompt Builder browser flow", () => {
  ...
  test("quick start recommends and applies a coherent strategy", async ({ page }) => {
    ...
    await expect(page.locator(".quick-education-button")).toHaveText("Start with the 5-minute guide");
    await expect(page.locator(".quick-start-button")).toHaveText("Open builder directly");
    await expect(page.locator(".quick-education-button .cta-icon")).toBeVisible();
    await expect(page.locator(".quick-start-button .cta-icon")).toBeVisible();
    await expect(page.locator(".quick-education-note")).toContainText("Know your profile?");
    ...
    await expect(page.locator(".quick-education-button")).toContainText("Mit dem 5-Minuten-Guide starten");
    await expect(page.locator(".quick-start-button")).toContainText("Builder direkt öffnen");
    await expect(page.locator(".quick-education-note")).toContainText("Profil bekannt?");
